				...
				if (arena.getGameType().equals(GameType.DUEL)) {
					...
					if (!arena.getDeadPlayers1().contains(sp.getUUID()) && !arena.getDeadPlayers2().contains(sp.getUUID())) {
						...
						if (arena.getState().equals(GameState.PAUSE)) {
							if (!arena.getResumeRequest().contains(sp.getUUID())) {
								arena.getResumeRequest().add(sp.getUUID());
							...
							arena.getResumeRequest().remove(sp.getUUID());
							Set<UUID> list = GameManager.getManager().leftPlayersToSomething(arena.getResumeRequest(), arena,false);
							
							for (UUID players : arena.getViewers()) {
								if (list.isEmpty()) {
									Bukkit.getPlayer(players).sendMessage("§b"+sp.getName()+" §chas cancelled their resume request.");
								} else {
									Bukkit.getPlayer(players).sendMessage("§b"+sp.getName()+" §chas cancelled their resume request §7(Left to accept: " + 
	...
	Set<UUID> list = GameManager.getManager().leftPlayersToSomething(arena.getResumeRequest(), arena,false);
		...
		for (UUID players : arena.getViewers()) {			
			if (!players.equals(sp.getUUID())) {
				Bukkit.getPlayer(players).sendMessage("§b"+sp.getName() + "§9 has requested to resume the match. To accept the request do /resume §7(Left to accept: " 
			+ Utils.getUtils().getPlayerNamesFromList(list) + ")");
				...
				Bukkit.getPlayer(players).sendMessage("§9You sent a resume request to your opponent.");
			}
		}
