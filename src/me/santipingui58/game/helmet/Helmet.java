package me.santipingui58.game.helmet;
...
public class Helmet {
	...
	public ItemStack getItem(UUID sp,boolean wearing) {
		...
		String helmet = DataPlayer.getPlayer().getHelmet(sp);
		
		if (p.hasPermission("splindux.helmet."+id.toLowerCase())) {
			if (helmet!=null && helmet.equalsIgnoreCase(getName())) {
		...
		if (wearing) {
			...
			if (helmet!=null && helmet.equalsIgnoreCase(getName()) && p.hasPermission("splindux.vip")) item.addUnsafeEnchantment(Enchantment.ARROW_FIRE, 1);
			
		} else {
			meta.addItemFlags(ItemFlag.HIDE_ENCHANTS);	
			meta.setLore(lore);
			item.setItemMeta(meta);	
			if (helmet!=null && helmet.equalsIgnoreCase(getName())) item.addUnsafeEnchantment(Enchantment.ARROW_FIRE, 1);
