				 ...
				 if (arena.getPlayers()!= null && arena.getPlayers().size()==0) continue;
		    		...
		    		if (arena.getState().equals(GameState.GAME)) {	
		    			...
		    			if (arena.getSpleefType().equals(SpleefType.SPLEEF)) {
		    				...
		    				if (arena.getGameType().equals(GameType.DUEL)) {
		    					if (arena.getTime()==10) {
		    						PlayerManager.getManager().sendSyncMessage(arena.getViewers(), "§bMini Spleef in 10 seconds.");
		    					}
		    					
		    				
		    				} else if (arena.getGameType().equals(GameType.FFA) && arena.getSpleefType().equals(SpleefType.SPLEEF)) {
		    					if (arena.getTime()==65) {
		    						PlayerManager.getManager().sendSyncMessage(arena.getViewers(), "§bAutomatic regeneration of the arena in 5 seconds.");
		    					}
		    					if (arena.getTime()==60) {
		    						arena.ice();
		    					...
		    					if (arena.getTime()<=100 && arena.getTime()%20==0 && arena.getTime()!=0) {
		    						...
		    						PlayerManager.getManager().sendSyncMessage(arena.getViewers(), text);
		    						arena.shrinkFFA();
		    					}
		    				}
						...
						if (arena.getTotalTime()>=60*arena.getTimedMaxTime() && !arena.isAtMiniSpleef()) {
							...
							if (diff>0) {
								...
								GameManager.getManager().endGameDuel(arena, null, GameEndReason.TIE);
									...
									PlayerManager.getManager().sendSyncMessage(arena.getViewers(), "§6§lGolden Point! §bWinner of this point wins the match.");
					...
					if (arena.getNextDecay()==0) {
						...
						PlayerManager.getManager().sendSyncMessage(arena.getViewers(), "§bDecay " + level + " of the arena in 10 seconds.");
					}
